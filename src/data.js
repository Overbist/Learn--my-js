...
const dataFaq = [
  ...
  },
  {
    ...
    answer: `
      Что такое es6 модули концепция модулей как способы организации джаваскрипт года существовала давно Когда предложение сложное и кода много самым лучшим путём его организации будет разбиение на отдельные небольшие файлы выполняющие какую-то функциональность либо группируя какие-либо данные по определенному признаку в каждом файле описывается какая-то небольшая структурная часть А в дальнейшем это всё собирается воедино модули позволяют разделить базу кода на несколько файлов для большего удобства обслуживания до появления из 6 модулей были две популярные модульные системы которые использовались для поддержки кода в джаваскрипт это команду сгс e.m.d. определение асинхронного модуля в браузере синтаксис использование модулей простой ключевое слово импорт используется для получения функциональности из друга файла в то время как экспорт используется для передачи функциональности из файла модули нужны для повышения читаемости и облегчения поддержки кода возможности повторного использования кода и сохранения чистоты глобального пространства имён
    `,
  ...
  },
  {
    ...
    answer: `
      Семён Что такое символы vc6 символ это отдельный тип данных который появился в стандарте ecmascript 2015 если говорить в целом-то символы это уникальное и неизменное значение которое позволяет добавлять уникальные свойства объектов или идентификатор и при этом не резервируется какие-нибудь строковое названия для этих свойств или идентификаторов создаётся они с помощью специальной функции символ без использования ключевого слова New и функции символ есть необязательные аргументы с помощью него можно указать имя для символа в последующем это имя можно использовать для описания символа следует отметить что Для различных символов можно использовать одно и тоже имя Однако Следует заметить что хоть имена и одинаковая но сами символы не будут равны Это означает что создать два одинаковых символа невозможно в обычной разработке символы практически не используются Однако их можно использовать при написании какой-то библиотеки для создания уникального пространства имён которое 100% будет гарантировать уникальность и не приведет к конфликту имён при интеграции этой библиотеки в приложении
    `,
